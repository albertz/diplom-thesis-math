...
\[ \Lambda := \Set{0 \le \SimpleMatrix{a}{b}{\overline b}{c} \in \Her_2(\curlO^\#)}{a,c \in \Z } \]
...
%\[ \FPrecisionLimit{S} = B \cdot (s + u - 2 \left| t \right|) , \] %WRONG! see 10.6. \F(S)
\[ \FPrecisionLimit{S} = \max(0, B(\min(s,u) - 2 |t|)) , \]
...
%We also have $as + cu \ge \max(a,c) (s + u)$.%WRONG!
Assuming $T \in \Lambda - \F$, we have $\max(a,c) \ge B$.
Case 1: Assume that we have $a \ge c$. Thus we have $a \ge B, |b|$ and
\[\tr(ST) \ge as + cu - 2a |t| = a(s - 2|t|)+ cu \ge B(s - 2 |t|) . \]
Case 2: Assume that we have $c \ge a$. Thus we have $c \ge B, |b|$ and
\[\tr(ST) \ge as + cu - 2 c |t| = c(u - 2|t|)+ as \ge B(u - 2 |t|) . \]
Thus, in both cases, we have
\[\tr(ST) \ge B(\min(s,u) - 2 |t|) . \]
We can use that as the limit, i.e.
\[ \FPrecisionLimit{S} = \max(0, B(\min(s,u) - 2 |t|)) . \qedhere \]
%For such $T$, we get
%\[ \tr(ST) \ge B \cdot (s + u - 2 \left| t \right|) . \] % WRONG!
%not relevant:
...
%thus we get
%\[ \FPrecisionLimit{S} = B \cdot (s + u - 2 \left| t \right|) . \qedhere \] %WRONG!
\endproof
\end{lemma}
\begin{remark}[On $\FPrecisionLimit{S}$]
\label{remark:on-fprecisionlimit}
% 10.6. \F(S) limit
Note that originially, we thought to have a better limit $\FPrecisionLimit{S}$. This limit is low and often $0$, thus we wont gain any information for many $S \in \PM_2(\curlO)$. Another way to improve it would be to change the precision set $\F$. For example, we could introduce another limit in $\F$ such that $\FPrecisionLimit{S}$ can be choosen better. For example
\[ \F'_{B,L} := \Set{T = [a,b,c] \in \Lambda}{0 \le a,c < B, |b| \le \frac{B}{L}} \subset \F \]
for $L \in \N$. That way, we still have
\[ \F'_{B_1,L} \subset \F'_{B_2,L} \;\; \forall\ B_1 \le B_2, \quad\quad \bigcup_{B \in \N} \F'_{B,L} = \Lambda . \]
And we would have the better precision Elliptic precision limit
\[ \F'_{B,L}(S) = \max(0, B (\min(s,u) - \tfrac{2}{L} |t|) ) . \]
...
\end{pmatrix} . \]
Given $0 = c_{2,3} = c_{\gamma_1,4} \cdot a_{1,3} + d_{\gamma_1,4} \cdot c_{1,3}$, where $a_{1,3} = 0$, we get $d_{\gamma_1,4} := 0$ and we can set $a_{\gamma_1,4} := 0$, $b_{\gamma_1,4} := 1$ and $c_{\gamma_1,4} := -1$ such that $\SmallMatrix{a_{\gamma_1,4}}{b_{\gamma_1,4}}{c_{\gamma_1,4}}{d_{\gamma_1,4}} = -J_1 \in \Sp_1(\curlO)$.
...
c_{4,3} &= c_{\gamma_3,4} \cdot a_{3,2} + d_{\gamma_3,4} \cdot \underbrace{c_{3,3}}_{=0} , \\
...
\end{proof}
\end{lemma}
Note that this proof gives explicit formulas to calculate $\gamma$ and $R$. This has been implemented in \ifilename{helpers.py} as \ifuncname{solveR}. A list of test cases have also been generated in the function \ifuncname{test_solveR} in \ifilename{tests.py} which also indirectly tests the \ifuncname{xgcd} and \ifuncname{divmod} implementation (described in \cref{xgcd}).

We want to analyze $\tilde{S}$ a bit more.
\begin{remark}[On $\tilde{S}$ in $solveR$]
\label{remark:on-solveR-tS}
Let $\tilde{S}_i$ the left upper 2-by-2 matrix of $\tilde{M}_i$.
We have
\[ \tilde{S}_2 = \SimpleMatrix{a_{\gamma_1,1}}{}{}{a_{\gamma_1,4}} a +
 \SimpleMatrix{b_{\gamma_1,1}}{}{}{b_{\gamma_1,4}} c S^{-1} , \]
\[ \tilde{S}_3 = A_{\gamma_2} \tilde{S}_2 , \]
and
\[ \tilde{S} = \tilde{S}_4 = 
\SimpleMatrix{a_{\gamma_3,1}}{}{}{a_{\gamma_3,4}} \tilde{S}_3 +
\SimpleMatrix{b_{\gamma_3,1}}{}{}{b_{\gamma_3,4}}
\SimpleMatrix{c_{3,1}}{c_{3,2}}{c_{3,3}}{c_{3,4}} . \]
Write $S = [s,t,u]$ and $l = \det(S)$, thus $S^{-1} = \frac{1}{l} [u,-t,s]$. 
Thus
\[ \tilde{S}_2 = \SimpleMatrix
{a_{\gamma_1,1} \cdot a + b_{\gamma_1,1} \cdot \tfrac{c}{l} u}  {- b_{\gamma_1,1} \cdot \tfrac{c}{l} t}
{- b_{\gamma_1,4} \cdot \tfrac{c}{l} \overline{t}}  {a_{\gamma_1,4} \cdot a + b_{\gamma_1,4} \cdot \tfrac{c}{l} s}
\]
and (note $a_{\gamma_1,4} = 0$, $b_{\gamma_1,4} = 1$)
\begin{align*}
\tilde{S} &=
\SimpleMatrix{a_{\gamma_3,1}}{}{}{1} A_{\gamma_2} \tilde{S}_2 +
\SimpleMatrix{b_{\gamma_3,1}}{}{}{}
\SimpleMatrix{}{c_{3,2}}{}{} \\
&= A_{\gamma_2} \SimpleMatrix
{a_{\gamma_3,1} \cdot a_{\gamma_1,1} \cdot a + b_{\gamma_1,1} \cdot \tfrac{c}{l} u}  {- a_{\gamma_3,1} \cdot b_{\gamma_1,1} \cdot \tfrac{c}{l} t}
{- b_{\gamma_1,4} \cdot \tfrac{c}{l} \overline{t}}  {a_{\gamma_1,4} \cdot a + b_{\gamma_1,4} \cdot \tfrac{c}{l} s}
+ \SimpleMatrix{}{b_{\gamma_3,1}\cdot c_{3,2}}{}{} \\
&= A_{\gamma_2} \SimpleMatrix
{a_{\gamma_3,1} \cdot a_{\gamma_1,1} \cdot a + b_{\gamma_1,1} \cdot \tfrac{c}{l} u}  {- a_{\gamma_3,1} \cdot b_{\gamma_1,1} \cdot \tfrac{c}{l} t}
{- \tfrac{c}{l} \overline{t}}  {\tfrac{c}{l} s}
+ \SimpleMatrix{}{b_{\gamma_3,1}\cdot c_{3,2}}{}{}
. \end{align*}
...
\end{align*}
where $L \in \N$ such that $\tr\left(T \tilde{S} S \overline{\tilde{S}}^T\right) \cdot l \in \N_0$ for all $T \in \Lambda$.
%Such $L \in \N$ exists, because $T, \tilde{S}, S, \overline{\tilde{S}}^T \ge 0$, thus $T \tilde{S} S \overline{\tilde{S}}^T \ge 0$.
We assume at this point that such $L \in \N$ exists. We analyze this in more detail in \cref{calcMatrixTrans}.
Then, we have
...
Note that we currently have implemented only the dimension formula for $\Delta = 3$. This follows from \cite[Theorem 7]{dern2003graded}, where we have
\[ \sum_{k=0}^{\infty} \dim \HSpace{k}{\Gamma, {\det}^k} t^k
= \frac{1 + t^{45}}{(1-t^4)(1-t^6)(1-t^9)(1-t^{10})(1-t^{12})} . \]
For $\Delta = 4$, we can use \cite[Corollary 9, Lemma 3]{dern2003graded} but we leave that and other cases open for further work.
