...
\item\label{NextSStep} Enumerate matrices $S \in \PM_2(\curlO)$ and for each matrix perform the following steps. Start with $\mathcal{M}_1 := \Q^{\invarF{\F}{\GL_2(\curlO)}}$.
...
\item
From \cref{remark:algo-mainstep1} and more detailed in \cref{calcMatrix}, we get a superspace $\mathcal{M}^H_S \subset \Q^{\invarF{\F}{\GL_2(\curlO)}}$ of $\FE(\HSpace{k}{\Gamma})$.
