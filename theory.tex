...
Thus, we can calculate the Fourier expansion coefficients
...
Let $S = [s,t,u]$ and $T = [a,b,c]$. Recall that $S \in \PM_2^T(\curlO)$. Then we have
...
\item\label{NextSStep} Enumerate matrices $S \in \PM_2^T(\curlO)$ and for each matrix perform the following steps.
