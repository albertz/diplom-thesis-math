...
With $[a,b,c] \in \F$, we have $0\le a,c<B$, thus there are only a finite number of possibilities. Because $0 \le [a,b,c]$, we get $ac - b^2 \ge 0$ and thus $b$ is also always limited. Thus, $\F$ is finite but it might be huge for even small numbers. Restricting the elements in $\F$ by $\GL_2(\curlO)$ invariation makes the set $\invar{\F}{\GL_2(\curlO)} \subseteq \F$ much smaller and better to handle in computer calculations. We use this set to identify a base of the finite dimension vector space $\invar{\Q^\F}{\GL_2(\curlO)}$.
