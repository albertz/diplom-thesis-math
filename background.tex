...
A matrix $Y \in \M_n(\C)$ is greater $0$ iff $\forall x \in \C^n - \Set{0} \colon Y[x] := \overline{x}^{\tr} Y x \in \R^+$. Such matrices are called the \defword{positive definitive matrices}, defined by $\PM_n = \Set{X \in \M_n}{X > 0}$.
