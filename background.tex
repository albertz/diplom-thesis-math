...
The \defword{denominator} of a matrix $Z \in \M_n(\Q)$ is the smallest number $x \in \N$ such that $x Z \in \M_n(\Z)$. We also write $\operatorname{denom}(Z) = x$. $1_n \in \M_n(\Z)$ denotes the \defword{identity matrix}. We use the \defword{Gauß notation} $[a,b,c] := \SmallMatrix{a}{b}{\bar b}{c} \in \M_n(\C)$.
