...
Thus, for $\SmallMatrix{a}{b}{c}{d} \in \Her_n(R)$, we have $a = \overline{a}$ and $d = \overline{d}$, i.e. $a,d \in R \cap \R$. We also have $c = \overline{b}$ and thus we introduce the shorter notation (\defword{Gauß notation}) $[a,b,c] := \SmallMatrix{a}{b}{\overline{b}}{c} \in \Her_n(\C)$ for $a,c \in \R$, $b \in \C$.
...
The \defword{denominator} of a matrix $Z \in \M_n(\Q)$ is the smallest number $x \in \N$ such that $x Z \in \M_n(\Z)$. We also write $\operatorname{denom}(Z) = x$. $1_n \in \M_n(\Z)$ denotes the \defword{identity matrix}.
