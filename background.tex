...
Let $f$ be a holomorphic function $\SiegelHalfPlane_1 \rightarrow \C$.
Let $\Gamma$ be a subgroup of $\Sp_1(\Z) = \SL_2(\Z)$. And let $k \in \Z$ be the \defword{weight} of our modular form. Let $M = \SmallMatrix{a}{b}{c}{d} \in \Gamma$ and $\tau \in \SiegelHalfPlane_1$. We write
\[ M \tau := \frac{a \tau + b}{b \tau + c} . \]
Modular forms are functions which are invariant with regard to a specific translation. In this case, the translation is given by $M$.
We define the \defword{translated function} $f | M \colon \SiegelHalfPlane_1 \rightarrow \C$ as
\[ (f | M) (\tau) :=  (c \tau + d)^{-k} \cdot f(M \tau) . \]

Then, an \defword{Elliptic modular form} with weight $k \in \Z$ over $\Gamma$ is a holomorphic function
\[ f \colon \SiegelHalfPlane_1 \rightarrow \C \]
with
\begin{align*}
(1) \ \ & f | M  = f \quad \forall \ M = \SmallMatrix{a}{b}{c}{d} \in \Gamma, \\
