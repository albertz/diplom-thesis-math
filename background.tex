...
A matrix $Y \in \M_n(\C)$ is greater $0$ iff $\forall x \in \C^n - \Set{0} \colon Y[x] := \overline{x}^{\tr} Y x \in \R^+$. Such matrices are called the \defword{positive definitive matrices}, defined by $\PM_n(R) = \Set{X \in \M_n(R)}{X > 0}$.
...
$\Sp_n(R)$ is also called the \defword{Hermitian modular group}.
...
Let $\SiegelHalfPlane_n := \Set{Z = X + iY \in \M_n^T(\C)}{Y > 0}$ be the \defword{Siegel upper half space}.
...
Let $\HalfPlane_n :=  \Set{ Z \in \M_n(\C) }{ \frac{1}{2 i} \left(Z - \overline{Z}^\tr \right) > 0}$ be the \defword{Hermitian uppper half space}.
