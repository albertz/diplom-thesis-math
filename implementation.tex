...
It also contains all the utilities for the parallelization. Details are described in \cref{impl:parallelization}.

\subsubsection{\ifilename{tests.py}}
This file contains some tests for some of the functions in \ifilename{algo.py}, \ifilename{helpers.py} and \ifilename{utils.py}.
...
We want to develop the algorithm analogously to \cref{calcMatrix}. We have developed the neccessary basics in \cref{remark:algo-mainstep2}. We have given some $S \in \PM_2(\curlO)$ and a cusp $c \in \Q$ of $\Gamma_0(l)$ with $l = \det(S)$.
