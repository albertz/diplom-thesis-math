...
As we are dealing with vector spaces, we can ignore the constant factor $\overline{\det(\tilde{S})}^k$, because for some vector space $\mathcal{V} \subset \Q^\F$, we have $(a \mapsto a[S]| M) (\mathcal{V}) = (a \mapsto \frac{a[S]| M}{\overline{\det(\tilde{S})}^k}) (\mathcal{V})$.
