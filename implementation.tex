...
In the C++ part, in the function \ifuncname{calcMatrixTrans}, we get the matrices $\tilde{S}$ and $\tilde{T}$ as described earlier (via \ifuncname{solveR}) and we have $S$ internally given. The function works very similar to \ifuncname{calcMatrix} (as described in \cref{calcMatrix}). The difference is that we prepare/return $P$ matrices, one for each factor $\zeta_P^i$ for $0 \le i < P$.

In Python, we get these $P$ matrices and transform them to the $(P-1)$ basis.
