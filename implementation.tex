...
In Python, in \ifuncname{modform_restriction_info} in the file \ifilename{algo.py}, we get an instance of that C++ class and call the function \ifuncname{calcMatrix}. This gives us the matrix $M$. Define its column module as
\[ \mathcal{M}_M := \Set{M a}{a \in \Q^\F} . \]
Via other methods in Sage, we can calculate the vector space $ \FE_{\F(S)}(\ESpace{k}{\Gamma_0(l)})$ of Fourier expansions of Elliptic modular forms to $\Gamma_0(l)$ where $l := det(S)$ and weight $2k$. Then consider the intersection
\[ \mathcal{M}'_M := \FE_{\F(S)}(\ESpace{2k}{\Gamma_0(l)} \cap \mathcal{M}_M . \]
Now, take them back to the Hermitian modular form space:
\[ \mathcal{M}^H_{M} := \Set{a \in \Q^\F}{Ma \in  \mathcal{M}'_M} . \]
In Sage, we can do that by using \ifuncname{solve_right} on the matrix $M$ and adding the right kernel of $M$.
