...
The function gets the fundamental discriminant \isymbname{D}, the Hermitian modula forms weight $k =$ \isymbname{HermWeight} and the precision limit $B_\F =$ \isymbname{B_cF} as its input and returns the vector space of Fourier expansions of Hermitian modular forms to the precision $B_\F$. The Fourier expansions are indexed by the reduced matrices of $\F$ (see \cref{remark:reducedCurlF} for details). This index list can also be returned by \ifuncname{herm_modform_indexset}.
...
The theory behind these functions is described in TODO-Ref...
