...
We want to construct a matrix $\hat{M}_{c,S}$ such that $\hat{M}_{c,S} \cdot a = a[S]|M_c$ for some given precisions.

Write $T = \SmallMatrix{t_1}{t_2}{t_3}{t_4}$, $S = [s_1,s_2,s_3]$ and $\tilde{S} = \SmallMatrix{u_1}{u_2}{u_3}{u_4}$, then
\begin{align*}
...
=\quad & {\left(s_{4} \overline{t_{2}} + t_{1} \overline{s_{2}}\right)} u_{2} + {\left(s_{4} t_{4} + t_{2} \overline{s_{2}} + 1\right)} u_{4} + {\left(s_{1} t_{2} + s_{2} t_{4}\right)} u_{3} + {\left(s_{1} t_{1} + s_{2} \overline{t_{2}} + 1\right)} u_{1} .
\end{align*}
