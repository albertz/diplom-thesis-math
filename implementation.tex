...
We want to construct a matrix $\hat{M}_{c,S}$ such that $\hat{M}_{c,S} \cdot a = a[S]|M_c$ for some given precision limit $\F_c(S)$ for the Elliptic modular forms (similar to $\FPrecisionLimit{S}$ as described in \cref{lemma:fprecisionlimit}).

Write $T = [t_1,t_2,t_4] \in \Lambda$, $S = [s_1,s_2,s_4] \in \PM_2(\curlO)$ and $\tilde{S} = \SmallMatrix{u_1}{u_2}{u_3}{u_4} \in \M_2(\K)$, then
\begin{align*}
